(function(){
	function bamboo(){};
	//初始化angularjs
	bamboo.angularApp = angular.module('repayIndex', []);
	...
	bamboo.contentHandleCtrl = function ($http,$rootScope,$scope,$compile) {
		...
		$scope.payForSomeone = function(j){
			if(j == 1){//自己支付
				location.href = IUSP.contextPath + '/repay/torepay.do';
			}else if(j == 2){
				location.href = IUSP.contextPath + '/repay/torepayforSomeone.do';
			}
		}
